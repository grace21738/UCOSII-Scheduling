		...
		if (OSTCBPrioTbl[idx] != (OS_TCB *)0 && OSTCBPrioTbl[idx] != (OS_TCB *)1)
		{
			...
			if (current->OSTCBDly==0)
			{
				...
				if ( current->ddl < mostUrgentDeadline)
				{
					...
					//printf("[New] mostUrgentPriority, mostUrgentDeadline: %d %d\n", mostUrgentPriority, mostUrgentDeadline);
				}
				// if (buffer_index < BUFFER_SIZE)
				//     printf("Compare: %d %d\n", current->OSTCBPrio, current->period);
			}
		}
...
#endif
    //2_Modification here
        //ªì©l¤ÆdeadlineÉOcompTime
        /*
        [Change UCOSII]
        */
        //INT32U *p = (INT32U *)pext;
        /*
        if( p != (INT32U *)0 ){
            //printf("AAAAAAAAAAAAa\n");
            ptcb->ddl = p[PERIOD];
            ptcb->compTime = p[COMP];
            ptcb->fullComp = p[COMP];
            ptcb->period = p[PERIOD];
        }
        else{
            ptcb->ddl = 0;
            ptcb->compTime = 0;
